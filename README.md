...
マリオの状態・地形・敵・アイテムを構造化し、Jev が実行可能な Skill を選び、フレーム単位の制御で操作する検証です。実行コードはマリオを対象とし、検証ドキュメントにはテトリス・スイカゲームとの比較も含みます。

## 3ゲームの検証レポート

セマンティックとkineticsの観点から、オントロジーが判断・実行にどう寄与し、どこで効果につながりにくかったかをまとめています。

1. [全体比較と実務への示唆](docs/ontology-game-study/01-overview.md)
2. [テトリス：判断フローと決定論的な実行](docs/ontology-game-study/02-tetris.md)
3. [マリオ：目的を持つSkillと継続制御](docs/ontology-game-study/03-mario.md)
4. [スイカゲーム：得点改善につながらなかった理由と学び](docs/ontology-game-study/04-suika.md)

文書内の「ローカル検証資料」は公開リポジトリに含まれない補足資料です。各レポートには判断に必要な結果とコード例を記載しています。
